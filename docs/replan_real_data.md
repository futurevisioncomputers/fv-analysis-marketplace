...
| 3 | ✅ Person-id entity resolution (`person_id` = salted hash of normalized name + last-10-digit phone, derived post-marker-strip / pre-mask; `person_enrollment_count`, `is_repeat_enrollment`). ✅ Phone-linked enquiry→admission conversion now lands via `enquiry_conversion` in run_sources output: person-grain, linked ACROSS sheets by person_id (enquired = is_enquiry anywhere, converted = is_admitted anywhere), reporting conversion_rate + `cross_source_conversions` (admission on a different sheet than the enquiry — the phone-link payoff). None when no enquiry frame carries person_id | Repeat-student + funnel metrics |
| 4 | ✅ New Analyst metrics + Monitoring hooks. Analyst METRIC_SPECS: `not_coming_rate`, `repeat_enrollment_rate`, `certificate_pending_rate`, `certificate_issue_lag_days` (+ `completion_rate` now backed by `is_completed`). Data Engineer emits `is_completed`/`is_not_coming` from completion_status; recon summary adds money-weighted `collection_efficiency = Σnet_paid/Σtotal_fees`, `default_aging` buckets (0-30/31-60/61-90/90+, as-of = ledger's latest receipt) + `overdue_90plus_amount`. Monitoring: business rules for not_coming/certificate_pending, area maps + LOWER_IS_BETTER + statistical-anomaly entries. ✅ collection_efficiency by dimension now lands via a new Analyst `ratio` kind (Σ numerator / Σ denominator, ratio-estimator 95% CI, two-period z on ratio SEs); Data Engineer derives per-enrollment `amount_collected = billed - pending` as the numerator; Monitoring gains a collection_efficiency business rule (<0.70 warn, <0.50 crit). ✅ `default_rate` now surfaced through Analyst (`rate`/`is_default`): Data Engineer flags per-enrollment `is_default = pending > 0` (negative pending is an overpayment, not a default); Monitoring adds a default_rate rule (>0.25 warn, >0.40 crit), area=Fees, LOWER_IS_BETTER. This is the enrollment-grain view; recon `default_aging` remains the money-weighted/overdue complement. ✅ duplicate-certificate + enquiry-backlog hooks now land: Data Engineer flags `is_duplicate_certificate` (repeated certificate_number serial, blanks ignored) and `is_enquiry_backlog` (unconverted enquiry aged > ENQUIRY_BACKLOG_DAYS=30 as-of the frame's latest enquiry); Analyst surfaces `duplicate_certificate_rate` + `enquiry_backlog_rate` (rate kind); Monitoring gates them (dup cert zero-tolerance >0 warn / >0.05 crit area=Certificates; backlog >0.30 warn / >0.50 crit area=Admissions), both LOWER_IS_BETTER. Still open: phone-linked enquiry→admission conversion (step 3 remainder) | Business value visible in report |
| 5 | ✅ PII mask v2. Data Engineer scrubs inline phone/email from RETAINED free-text columns (status_reason, description, remark/note/reason/… headers) after column-level hashing — permissive phone pattern `\+?\d[\d\s().\-]{8,14}\d` gated by a ≥10-digit count so dates/pincodes/ids/amounts in the same cell survive. Report defence-in-depth widened: `_contains_mobile` catches bare 10-digit AND formatted/international phones, but ignores separator-free Chart.js numeric literals (epoch ms, large values) so valid reports aren't withheld. `llm_client.mask_text` broadened to the same guarded pattern | Correctness/safety |
| 6 | Hand institute the C-checklist; wait one intake cycle | Creates labels |
| 7 | 🟡 Prediction agent (4.5) — SCAFFOLDED. `agents/prediction_agent.py`: dependency-free categorical Naive Bayes (Laplace-smoothed, deterministic 1-in-5 holdout keyed by person_id, explainable via per-feature-value churn log-likelihood ratios). Trains the churn model NOW on the timetable completion label (terminal-only: completed vs not_coming; `active` rows censored/excluded). Behind the honesty gate: blocks when labels absent / single-class / < MIN_LABELED_ROWS(40) / < MIN_PER_CLASS(10). Reports holdout accuracy vs majority baseline + accuracy_lift + top_risk_factors + caveats. Still needs step 6 C-changes for the OTHER models: fee-default (due_date/EMI features) and lead-conversion propensity | Churn model live now; default/propensity models after 6 |
