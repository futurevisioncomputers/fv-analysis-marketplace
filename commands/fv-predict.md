---
description: Train an explainable churn model on the completion labels, or refuse and say why (Prediction (stage 4.5, optional)), then stop at a checkpoint
argument-hint: [csv-or-xlsx-path-or-sheet-url] [business question]
allowed-tools: Bash, Read
---

Run Prediction (stage 4.5, optional) for the user and stop there. Follow the `fv-predict` skill for
...
2. Run it — prerequisites back-fill automatically:
   ...
   python "${CLAUDE_PLUGIN_ROOT}/scripts/run_stage.py" \
     [--csv "<source>"] [--question "<question>"] --stage predict --json
   ```
