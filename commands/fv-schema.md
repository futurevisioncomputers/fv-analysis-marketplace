---
description: Validate the sheets against the questions and plan any joins (Schema / Source Plan (stage 2.5)), then stop at a checkpoint
argument-hint: [csv-or-xlsx-path-or-sheet-url] [business question]
allowed-tools: Bash, Read
---

Run Schema / Source Plan (stage 2.5) for the user and stop there. Follow the `fv-schema` skill for
...
2. Run it — prerequisites back-fill automatically:
   ...
   python "${CLAUDE_PLUGIN_ROOT}/scripts/run_stage.py" \
     [--csv "<source>"] [--question "<question>"] --stage schema --json
   ```
