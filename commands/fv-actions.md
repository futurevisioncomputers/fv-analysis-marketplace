---
description: Propose prioritized, owner-tagged actions from the insights (Recommendation (stage 6.5)), then stop at a checkpoint
argument-hint: [csv-or-xlsx-path-or-sheet-url] [business question]
allowed-tools: Bash, Read
---

Run Recommendation (stage 6.5) for the user and stop there. Follow the `fv-actions` skill for
...
2. Run it — prerequisites back-fill automatically:
   ...
   python "${CLAUDE_PLUGIN_ROOT}/scripts/run_stage.py" \
     [--csv "<source>"] [--question "<question>"] --stage recommend --json
   ```
