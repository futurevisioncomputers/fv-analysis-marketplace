---
description: Register KPI hooks and evaluate them, raising threshold alerts (Monitoring (stage 7)), then stop at a checkpoint
argument-hint: [csv-or-xlsx-path-or-sheet-url] [business question]
allowed-tools: Bash, Read
---

Run Monitoring (stage 7) for the user and stop there. Follow the `fv-monitor` skill for
...
2. Run it — prerequisites back-fill automatically:
   ...
   python "${CLAUDE_PLUGIN_ROOT}/scripts/run_stage.py" \
     [--csv "<source>"] [--question "<question>"] --stage monitor --json
   ```
